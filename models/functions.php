<?php
    ...
    function deleteProduct($id_product){
        ...
        $statement = $pdo->prepare($abfrage);
        if($statement->execute([$id_product]) ) {
                echo "<div class='alert alert-success' role='alert'>  Produit supprimé ! </div>";
            }
        else{
            echo "<div class='alert alert-danger' role='alert'> suppression impossible \n Try again ! </div>";
        }
    }
    ...
    //notify client 
    function notifyClient($cmd,$status){
        $to = "kamerfooditalia@gmail.com,".$cmd->getMail()."";
            $subject = "Livraison Commande Kmerfood";
            $product = getProductById($cmd->getIdProduct());
            $footer_msg = "";
            if($status == "En route")
                $footer_msg = " est en chemin. ";
            else
                if($status == "En route")
                    $footer_msg = " a été livré .";
            ...
            <p/><div class='alert alert-secondary' role='alert'> ".$footer_msg." </div><p/> <h4>Merci pour votre confiance!</h4>
            </body>
            </html>
            ";
