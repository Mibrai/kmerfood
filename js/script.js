...
function updateCommand(id_cmd){
    var status = document.getElementById("status"+id_cmd).value;
    var delivry = document.getElementById("delivry"+id_cmd).value;
    console.log(status +" date delivry "+ delivry+"  id cmd " + id_cmd);
        $.get("../models/updateCommand.php?id_cmd="+id_cmd+"&status="+status+"&delivry="+delivry, function(rep){
            console.log(rep);
            $("#col_result"+id_cmd).html(rep);
        });
...
function updateProduct(id_product){
    ...
    var new_price = $("#new_price"+id_product).val();
        $.get("../models/updateProduct.php?id_product="+id_product+"&old_price="+old_price+"&new_price="+new_price, function(rep){
            console.log(rep);
            $("#col_result"+id_product).html(rep);
...
function deleteProduct(id_product){
        $.get("../models/deleteProduct.php?id_product="+id_product, function(rep){
            console.log(rep);
            $("#col_result"+id_product).html(rep);
